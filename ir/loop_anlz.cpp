...
void LoopAnalyzer::transform(ImperativeForFlow *v) {
  ...
  if (att_att) {
    ...
    auto *ass = cast<CallInstr>(cast<SeriesFlow>(inner_loop->getBody())
                                    ->back()); // assignment op inside the inner loop
    auto *right_side =
        cast<CallInstr>(ass->back()); // the right side of the assignment operation
    auto *oper = util::getFunc(right_side->getCallee());
    auto op = oper->getUnmangledName(); // the name of the assignment function (add,
                                        // sub, min or max)
    ...
    if (call_end_outer) { // writing the end condition of the loop
      ...
      if (final_end == "len") {
        std::vector<codon::ir::Var *> len_arg =
            call_end_outer->back()->getUsedVariables();
        ...
        std::vector<codon::ir::Var *> len_arg =
            cast<CallInstr>(call_end_outer->front())->back()->getUsedVariables();
        auto len_arg_name = len_arg[0]->getName();
        if (len_arg_name == pf_arg1) {
          prep_02 = "-1";
        } else {
    ...
    if (call_end_inner) { // writing the end condition of the loop
      ...
      if (final_end_inner == "len") {
        std::vector<codon::ir::Var *> len_arg_inner =
            call_end_inner->back()->getUsedVariables();
        ...
        std::vector<codon::ir::Var *> len_arg =
            cast<CallInstr>(call_end_inner->front())->back()->getUsedVariables();
        auto len_arg_name = len_arg[0]->getName();
        if (len_arg_name == pf_arg1) {
          prep_06 = "-1";
        } else {
    ...
    std::map<std::string, std::string> prep_attributes{
        {"prep_00", prep_00}, {"prep_01", prep_01}, {"prep_02", prep_02},
        {"prep_03", prep_03}, {"prep_04", prep_04}, {"prep_05", prep_05},
        {"prep_06", prep_06}, {"prep_07", prep_07}};
    ...
    if (chk == NULL) {
      ...
      auto *main_true_branch =
          cast<IfFlow>(cast<SeriesFlow>(inner_loop->getBody())->back())->getCond();
      ...
      if (and_check == "false" || or_check == "true") {
        ...
        auto *left_lhs = cast<CallInstr>(
            left_cond_vars[0]); // the last op of the lhs of first condition
        if (left_lhs == NULL) {
          ...
          auto *left_lhs_lhs_func = cast<CallInstr>(
              left_lhs_lhs); // the second to last op of the lhs of first condition
          if (left_lhs_lhs_func == NULL) {
            std::vector<codon::ir::Var *> var_lhs_lhs =
                left_lhs_lhs->getUsedVariables();
            ...
            std::vector<codon::ir::Var *> var_lhs_rhs =
                left_lhs_rhs->getUsedVariables();
            ...
            auto neg_test_name =
                util::getFunc(left_lhs_lhs_func->getCallee())->getUnmangledName();
            auto *first_lhs_operand = left_lhs_lhs_func->back();
            std::vector<codon::ir::Var *> first_var =
                first_lhs_operand->getUsedVariables();
        ...
        int build_flag = 0;
        auto *right_operator =
            util::getFunc(cast<CallInstr>(right_cond_vars[0])->getCallee());
            ...
            if (right_op_name == "__ge__") {
              ...
              if (right_op_name == "__lt__") {
                ...
                if (right_op_name == "__le__") {
                  ...
                  build_flag = 1;
                  right_operator =
                      util::getFunc(cast<CallInstr>(right_cond)->getCallee());
        ...
        auto *right = cast<CallInstr>(right_cond_vars[0])
                          ->front(); // the left hand side of the right condition
        ...
        if (right_lhs == NULL) {
          ...
          auto r_last_lhs_op =
              util::getFunc(right_lhs->getCallee())->getUnmangledName();
          auto *right_lhs_lhs = right_lhs->front();
          auto *right_lhs_lhs_func = cast<CallInstr>(
              right_lhs_lhs); // the second to last op of the lhs of first condition
          if (right_lhs_lhs_func == NULL) {
            std::vector<codon::ir::Var *> r_var_lhs_lhs =
                right_lhs_lhs->getUsedVariables();
            ...
            std::vector<codon::ir::Var *> r_var_lhs_rhs =
                right_lhs_rhs->getUsedVariables();
            ...
            auto neg_test_name =
                util::getFunc(right_lhs_lhs_func->getCallee())->getUnmangledName();
            auto *r_first_lhs_operand = right_lhs_lhs_func->back();
            std::vector<codon::ir::Var *> r_first_var =
                r_first_lhs_operand->getUsedVariables();
            ...
            auto *right_lhs_rhs = right_lhs->back();
            std::vector<codon::ir::Var *> r_second_var =
                right_lhs_rhs->getUsedVariables();
        ...
        auto *left_lhs = cast<CallInstr>(
            main_true_branch); // the last op of the lhs of first condition
      ...
      std::map<std::string, std::string> bw_attributes{
          {"bw_00", bw_00}, {"bw_01", bw_01}, {"bw_02", bw_02},
          {"bw_03", bw_03}, {"bw_04", bw_04}, {"bw_05", bw_05},
          {"bw_06", bw_06}, {"bw_07", bw_07}, {"bw_08", bw_08}};
    ...
    auto *ass = cast<CallInstr>(cast<SeriesFlow>(inner_loop->getBody())
                                    ->back()); // assignment op inside the inner loop
    if (flag == 0) {
      auto *main_else_branch =
          cast<IfFlow>(cast<SeriesFlow>(inner_loop->getBody())->back())
              ->getFalseBranch();
      ass = cast<CallInstr>(cast<SeriesFlow>(main_else_branch)
                                ->back()); // assignment op inside the inner loop
    }
    auto *right_side =
        cast<CallInstr>(ass->back()); // the right side of the assignment operation
    std::vector<codon::ir::Var *> left_side =
        cast<CallInstr>(ass->front())->front()->getUsedVariables();
    ...
    if (call_end_inner) { // writing the end condition of the loop
      ...
      if (final_end_inner == "len") {
        // auto *len_arg_fr =
        // cast<CallInstr>(cast<CallInstr>(call_end_inner)->front())->front();
    ...
    if (op == "max" || op == "min") {
      ...
      auto *r_end = cast<CallInstr>(right_side->front())
                        ->back(); // the last argument of the main function (min or max)
      int main_args = 0;
      if (r_end != r_mid[1]) {
        main_args = 1;
      }
      ...
      if (arg1_inst == NULL) {
        ...
        auto max_att_1 =
            util::hasAttribute(max_st_1, "std.vectron.dispatcher.vectron_max");
        if (arg1_func_name == "__add__" || arg1_func_name == "__sub__" ||
            arg1_func_name == "__mul__" || arg1_func_name == "__div__") {
          params_09 = "1";
        } else {
          if (max_att_1) {
            params_09 = "0";
      ...
      if (arg2_inst == NULL) {
        ...
        auto max_att_2 =
            util::hasAttribute(max_st_2, "std.vectron.dispatcher.vectron_max");
        if (arg2_func_name == "__add__" || arg2_func_name == "__sub__" ||
            arg2_func_name == "__mul__" || arg2_func_name == "__div__") {
          params_10 = "1";
        } else {
          if (max_att_2) {
            params_10 = "0";
      ...
      if (main_args == 0) {
        ...
        if (arg3_inst == NULL) {
          ...
          auto arg3_func_name =
              util::getFunc(arg3_inst->getCallee())->getUnmangledName();
          auto *max_st_3 = util::getFunc(arg3_inst->getCallee());
          auto max_att_3 =
              util::hasAttribute(max_st_3, "std.vectron.dispatcher.vectron_max");
          if (arg3_func_name == "__add__" || arg3_func_name == "__sub__" ||
              arg3_func_name == "__mul__" || arg3_func_name == "__div__") {
            params_11 = "1";
          } else {
            if (max_att_3) {
              params_11 = "0";
      ...
      if (arg1_inst == NULL) {
        ...
        auto max_att_4 =
            util::hasAttribute(max_st_4, "std.vectron.dispatcher.vectron_max");
        if (max_att_4) {
          params_12 = "0";
          params_13 = "0";
          params_14 = "0";
          ...
          if (mx_arg_1_2 == NULL) {
            ...
            auto mx_arg_1_2_func =
                util::getFunc(mx_arg_1_2->getCallee())->getUnmangledName();
          ...
          if (mx_arg_1_3 == NULL) {
            ...
            auto mx_arg_1_3_func =
                util::getFunc(mx_arg_1_3->getCallee())->getUnmangledName();
          ...
          auto mx_arg_1_4_name = util::getFunc(cast<CallInstr>(mx_arg_1_4)->getCallee())
                                     ->getUnmangledName();
          if (mx_arg_1_4_name == "__add__" || mx_arg_1_4_name == "__sub__") {
            ...
            auto *mx_arg_1_4_left_part_1_1 =
                cast<CallInstr>(mx_arg_1_4_left_part_1)->front();
            std::vector<codon::ir::Var *> part_1_lst =
                mx_arg_1_4_left_part_1_1->getUsedVariables();
            auto part_1_lst_name = part_1_lst[0]->getName();
            mx1_03 = part_1_lst_name;
            auto *mx_arg_1_4_left_part_1_2 =
                cast<CallInstr>(mx_arg_1_4_left_part_1)->back();
            if (cast<CallInstr>(mx_arg_1_4_left_part_1_2) == NULL) {
              mx1_04 = "0";
            } else {
              auto mx_arg_1_4_left_inst =
                  util::getFunc(cast<CallInstr>(mx_arg_1_4_left_part_1_2)->getCallee());
            ...
            if (cast<CallInstr>(mx_arg_1_4_part_2) == NULL) {
              ...
              auto *mx_arg_1_4_left_part_2 =
                  util::getFunc(cast<CallInstr>(mx_arg_1_4_part_2)->getCallee());
          ...
          auto mx_arg_1_5_name = util::getFunc(cast<CallInstr>(mx_arg_1_5)->getCallee())
                                     ->getUnmangledName();
          if (mx_arg_1_5_name == "__add__" || mx_arg_1_5_name == "__sub__") {
            ...
            auto *mx_arg_1_5_left_part_1_1 =
                cast<CallInstr>(mx_arg_1_5_left_part_1)->front();
            std::vector<codon::ir::Var *> part_1_lst =
                mx_arg_1_5_left_part_1_1->getUsedVariables();
            auto part_1_lst_name = part_1_lst[0]->getName();
            mx1_07 = part_1_lst_name;
            auto *mx_arg_1_5_left_part_1_2 =
                cast<CallInstr>(mx_arg_1_5_left_part_1)->back();
            if (cast<CallInstr>(mx_arg_1_5_left_part_1_2) == NULL) {
              mx1_08 = "0";
            } else {
              auto mx_arg_1_5_left_inst =
                  util::getFunc(cast<CallInstr>(mx_arg_1_5_left_part_1_2)->getCallee());
            ...
            if (cast<CallInstr>(mx_arg_1_5_part_2) == NULL) {
              ...
              auto *mx_arg_1_5_left_part_2 =
                  util::getFunc(cast<CallInstr>(mx_arg_1_5_part_2)->getCallee());
          ...
          std::map<std::string, std::string> mx1_attributes{
              {"mx1_00", mx1_00}, {"mx1_01", mx1_01}, {"mx1_02", mx1_02},
              {"mx1_03", mx1_03}, {"mx1_04", mx1_04}, {"mx1_05", mx1_05},
              {"mx1_06", mx1_06}, {"mx1_07", mx1_07}, {"mx1_08", mx1_08},
              {"mx1_09", mx1_09}, {"mx1_10", mx1_10}};
          globalAttributes["mx1"] = mx1_attributes;
        } else {
          auto arg1_func_name =
              util::getFunc(arg1_inst->getCallee())->getUnmangledName();
          if (arg1_func_name == "__add__" || arg1_func_name == "__sub__" ||
              arg1_func_name == "__mul__" || arg1_func_name == "__div__") {
            auto *arg1_op =
                cast<CallInstr>(arg1_inst->front()); // the left side operand
            auto *arg1_fr = cast<CallInstr>(
                arg1_op->front());             // the left side variable + its row index
            ...
            auto *arg1_row_instr =
                cast<CallInstr>(arg1_row); // is arg1 row index a math op?
            auto *arg1_col_instr =
                cast<CallInstr>(arg1_col); // is arg1 row index a math op?
            auto arg1_fr_name =
                arg1_var_name[0]->getName(); // left side variable's name
            ...
            std::string arg1_row_var_name =
                ""; // taking care of the row index of the first (left side) operand
            if (arg1_row_instr != NULL) {
              ...
              std::vector<codon::ir::Var *> arg1_row_var =
                  arg1_row_instr->front()->getUsedVariables();
            ...
            std::string arg1_col_var_name =
                ""; // taking care of the column index of the first (left side) operand
            if (arg1_col_instr != NULL) {
              ...
              std::vector<codon::ir::Var *> arg1_col_var =
                  arg1_col_instr->front()->getUsedVariables();
              ...
              if (arg1_num_col_inst == NULL) {
                ...
                std::vector<codon::ir::Var *> arg_1_num_col_left_vars =
                    arg_1_num_col_left->getUsedVariables();
                ...
                if (arg_1_num_col_right_inst == NULL) {
                  std::vector<codon::ir::Var *> arg_1_num_col_right_vars =
                      arg_1_num_col_right->getUsedVariables();
                  auto arg_1_num_col_right_name =
                      arg_1_num_col_right_vars[0]->getName();
                  ...
                  std::vector<codon::ir::Var *> arg1_num_col_right_left_vars =
                      arg1_num_col_right_left->getUsedVariables();
                  auto arg1_num_col_right_left_name =
                      arg1_num_col_right_left_vars[0]->getName();
                  ...
                  auto *arg1_num_col_right_func =
                      util::getFunc(arg_1_num_col_right_inst->getCallee());
                  auto arg1_num_col_right_func_name =
                      arg1_num_col_right_func->getUnmangledName();
                  ...
                  std::string oss_arg1_num_col_right_right_str =
                      oss_arg1_num_col_right_right.str();
              ...
              if (cast<CallInstr>(arg1_const) == NULL) {
                if (cast<TernaryInstr>(arg1_const) == NULL) {
                  ...
                  if (cast<CallInstr>(arg_1_1_index) != NULL) {
                    ...
                    auto arg_1_1_index_func_name =
                        arg_1_1_index_func->getUnmangledName();
                  ...
                  if (cast<CallInstr>(arg_1_2_index) != NULL) {
                    ...
                    auto arg_1_2_index_func_name =
                        arg_1_2_index_func->getUnmangledName();
                  ...
                  std::map<std::string, std::string> arg1_attributes{
                      {"arg1_00", arg1_00}, {"arg1_01", arg1_01}, {"arg1_02", arg1_02},
                      {"arg1_03", arg1_03}, {"arg1_04", arg1_04}, {"arg1_05", arg1_05}};
                ...
                auto match_att_1 = util::hasAttribute(
                    m_call_1_func, "std.vectron.dispatcher.vectron_cmp");
                if (match_att_1) {
                  std::vector<codon::ir::Value *> body_1 =
                      m_call_1_func->getUsedValues();
                  auto match_arg_1 = m_call_1_func->arg_front()->getName();
                  auto match_arg_2 = m_call_1_func->arg_back()->getName();

                  auto *true_return_1 =
                      cast<IfFlow>(cast<SeriesFlow>(body_1[0])->back())
                          ->getTrueBranch();

                  auto *elif_branch_1_series =
                      cast<IfFlow>(cast<SeriesFlow>(body_1[0])->back())
                          ->getFalseBranch();
                  auto *elif_branch_1 =
                      cast<IfFlow>(cast<SeriesFlow>(elif_branch_1_series)->back())
                          ->getCond();
                  auto *elif_return_1 =
                      cast<IfFlow>(cast<SeriesFlow>(elif_branch_1_series)->back())
                          ->getTrueBranch();
                  ...
                  std::vector<codon::ir::Var *> elif_arg_1 =
                      elif_cond_1_call->front()->getUsedVariables();
                  std::vector<codon::ir::Var *> elif_arg_2 =
                      elif_cond_1_call->back()->getUsedVariables();
                  ...
                  std::string arg1_05 = "";
                  arg1_00 = "0";
                  std::vector<codon::ir::Value *> all_args_1 =
                      m_call_1->getUsedValues();
                  auto *a_arg = all_args_1[1];
                  auto *b_arg = all_args_1[2];
                  auto *am_arg = all_args_1[3];
                  if (elif_cond_func_name == "__eq__" &&
                      (elif_arg_1_name == match_arg_1 ||
                       elif_arg_1_name == match_arg_2) &&
                      ((elif_arg_2_name == match_arg_1 ||
                        elif_arg_2_name == match_arg_2))) {
                    auto *return_val_1_1 =
                        cast<ReturnInstr>(cast<SeriesFlow>(elif_return_1)->back())
                            ->getValue();
                    auto *return_val_1_2 =
                        cast<ReturnInstr>(cast<SeriesFlow>(true_return_1)->back())
                            ->getValue();
                    auto *return_val_1_3 =
                        cast<ReturnInstr>(cast<SeriesFlow>(else_branch_1)->back())
                            ->getValue();
                    std::vector<codon::ir::Var *> ret_val_1 =
                        return_val_1_1->getUsedVariables();
                    auto ret_val_1_name = ret_val_1[0]->getName();
                    std::vector<codon::ir::Var *> ret_val_2 =
                        return_val_1_2->getUsedVariables();
                    auto ret_val_2_name = ret_val_2[0]->getName();
                    std::vector<codon::ir::Var *> ret_val_3 =
                        return_val_1_3->getUsedVariables();
            ...
            auto *arg1_fr = cast<CallInstr>(
                arg1_inst->front());            // the left side operand + its row index
            auto *arg1_var = arg1_fr->front();  // the left side variable
            auto *arg1_row = arg1_fr->back();   // the left side row index
      ...
      if (arg2_inst == NULL) {
        ...
        auto max_att_5 =
            util::hasAttribute(max_st_5, "std.vectron.dispatcher.vectron_max");
        if (max_att_5) {
          params_18 = "0";
          params_19 = "0";
          params_20 = "0";
          ...
          if (mx_arg_2_2 == NULL) {
            ...
            auto mx_arg_2_2_func =
                util::getFunc(mx_arg_2_2->getCallee())->getUnmangledName();
          ...
          if (mx_arg_2_3 == NULL) {
            ...
            auto mx_arg_2_3_func =
                util::getFunc(mx_arg_2_3->getCallee())->getUnmangledName();
          ...
          auto mx_arg_2_4_name = util::getFunc(cast<CallInstr>(mx_arg_2_4)->getCallee())
                                     ->getUnmangledName();
          if (mx_arg_2_4_name == "__add__" || mx_arg_2_4_name == "__sub__") {
            auto *mx_arg_2_4_left = mx_arg_2_4->front();
            auto *mx_arg_2_4_left_part_1 = cast<CallInstr>(mx_arg_2_4_left)->front();
            auto *mx_arg_2_4_left_part_1_1 =
                cast<CallInstr>(mx_arg_2_4_left_part_1)->front();
            std::vector<codon::ir::Var *> part_1_lst =
                mx_arg_2_4_left_part_1_1->getUsedVariables();
            auto part_1_lst_name = part_1_lst[0]->getName();
            mx2_03 = part_1_lst_name;
            auto *mx_arg_2_4_left_part_1_2 =
                cast<CallInstr>(mx_arg_2_4_left_part_1)->back();
            if (cast<CallInstr>(mx_arg_2_4_left_part_1_2) == NULL) {
              mx2_04 = "0";
            } else {
              auto mx_arg_2_4_left_inst =
                  util::getFunc(cast<CallInstr>(mx_arg_2_4_left_part_1_2)->getCallee());
            ...
            if (cast<CallInstr>(mx_arg_2_4_part_2) == NULL) {
              ...
              auto *mx_arg_2_4_left_part_2 =
                  util::getFunc(cast<CallInstr>(mx_arg_2_4_part_2)->getCallee());
          ...
          auto mx_arg_2_5_name = util::getFunc(cast<CallInstr>(mx_arg_2_5)->getCallee())
                                     ->getUnmangledName();
          if (mx_arg_2_5_name == "__add__" || mx_arg_2_5_name == "__sub__") {
            ...
            auto *mx_arg_2_5_left_part_1_1 =
                cast<CallInstr>(mx_arg_2_5_left_part_1)->front();
            std::vector<codon::ir::Var *> part_1_lst =
                mx_arg_2_5_left_part_1_1->getUsedVariables();
            auto part_1_lst_name = part_1_lst[0]->getName();
            mx2_07 = part_1_lst_name;
            auto *mx_arg_2_5_left_part_1_2 =
                cast<CallInstr>(mx_arg_2_5_left_part_1)->back();
            if (cast<CallInstr>(mx_arg_2_5_left_part_1_2) == NULL) {
              mx2_08 = "0";
            } else {
              auto mx_arg_2_5_left_inst =
                  util::getFunc(cast<CallInstr>(mx_arg_2_5_left_part_1_2)->getCallee());
            ...
            if (cast<CallInstr>(mx_arg_2_5_part_2) == NULL) {
              ...
              auto *mx_arg_2_5_left_part_2 =
                  util::getFunc(cast<CallInstr>(mx_arg_2_5_part_2)->getCallee());
          ...
          std::map<std::string, std::string> mx2_attributes{
              {"mx2_00", mx2_00}, {"mx2_01", mx2_01}, {"mx2_02", mx2_02},
              {"mx2_03", mx2_03}, {"mx2_04", mx2_04}, {"mx2_05", mx2_05},
              {"mx2_06", mx2_06}, {"mx2_07", mx2_07}, {"mx2_08", mx2_08},
              {"mx2_09", mx2_09}, {"mx2_10", mx2_10}};
          globalAttributes["mx2"] = mx2_attributes;
        } else {
          auto arg2_func_name =
              util::getFunc(arg2_inst->getCallee())->getUnmangledName();
          if (arg2_func_name == "__add__" || arg2_func_name == "__sub__" ||
              arg2_func_name == "__mul__" || arg2_func_name == "__div__") {
            auto *arg2_op =
                cast<CallInstr>(arg2_inst->front()); // the left side operand
            auto *arg2_fr = cast<CallInstr>(
                arg2_op->front());             // the left side variable + its row index
            ...
            auto *arg2_row_instr =
                cast<CallInstr>(arg2_row); // is arg2 row index a math op?
            auto *arg2_col_instr =
                cast<CallInstr>(arg2_col); // is arg2 row index a math op?
            auto arg2_fr_name =
                arg2_var_name[0]->getName(); // left side variable's name
            ...
            if (arg2_row_instr != NULL) {
              ...
              std::vector<codon::ir::Var *> arg2_row_var =
                  arg2_row_instr->front()->getUsedVariables();
            ...
            if (arg2_col_instr != NULL) {
              ...
              std::vector<codon::ir::Var *> arg2_col_var =
                  arg2_col_instr->front()->getUsedVariables();
              ...
              if (arg2_num_col_inst == NULL) {
                ...
                std::vector<codon::ir::Var *> arg_2_num_col_left_vars =
                    arg_2_num_col_left->getUsedVariables();
                ...
                if (arg_2_num_col_right_inst == NULL) {
                  std::vector<codon::ir::Var *> arg_2_num_col_right_vars =
                      arg_2_num_col_right->getUsedVariables();
                  auto arg_2_num_col_right_name =
                      arg_2_num_col_right_vars[0]->getName();
                  ...
                  std::vector<codon::ir::Var *> arg2_num_col_right_left_vars =
                      arg2_num_col_right_left->getUsedVariables();
                  auto arg2_num_col_right_left_name =
                      arg2_num_col_right_left_vars[0]->getName();
                  ...
                  auto *arg2_num_col_right_func =
                      util::getFunc(arg_2_num_col_right_inst->getCallee());
                  auto arg2_num_col_right_func_name =
                      arg2_num_col_right_func->getUnmangledName();
                  ...
                  std::string oss_arg2_num_col_right_right_str =
                      oss_arg2_num_col_right_right.str();
              ...
              if (cast<CallInstr>(arg2_const) == NULL) {
                if (cast<TernaryInstr>(arg2_const) == NULL) {
                  ...
                  if (cast<CallInstr>(arg_2_1_index) != NULL) {
                    ...
                    auto arg_2_1_index_func_name =
                        arg_2_1_index_func->getUnmangledName();
                  ...
                  if (cast<CallInstr>(arg_2_2_index) != NULL) {
                    ...
                    auto arg_2_2_index_func_name =
                        arg_2_2_index_func->getUnmangledName();
                  ...
                  std::map<std::string, std::string> arg2_attributes{
                      {"arg2_00", arg2_00}, {"arg2_01", arg2_01}, {"arg2_02", arg2_02},
                      {"arg2_03", arg2_03}, {"arg2_04", arg2_04}, {"arg2_05", arg2_05}};
                ...
                auto match_att_2 = util::hasAttribute(
                    m_call_2_func, "std.vectron.dispatcher.vectron_cmp");
                if (match_att_2) {
                  std::vector<codon::ir::Value *> body_2 =
                      m_call_2_func->getUsedValues();
                  auto match_arg_1 = m_call_2_func->arg_front()->getName();
                  auto match_arg_2 = m_call_2_func->arg_back()->getName();

                  // auto *true_branch_2 =
                  // cast<IfFlow>(cast<SeriesFlow>(body_2[0])->back())->getCond();
                  auto *true_return_2 =
                      cast<IfFlow>(cast<SeriesFlow>(body_2[0])->back())
                          ->getTrueBranch();

                  auto *elif_branch_2_series =
                      cast<IfFlow>(cast<SeriesFlow>(body_2[0])->back())
                          ->getFalseBranch();
                  auto *elif_branch_2 =
                      cast<IfFlow>(cast<SeriesFlow>(elif_branch_2_series)->back())
                          ->getCond();
                  auto *elif_return_2 =
                      cast<IfFlow>(cast<SeriesFlow>(elif_branch_2_series)->back())
                          ->getTrueBranch();
                  ...
                  std::vector<codon::ir::Var *> elif_arg_1 =
                      elif_cond_2_call->front()->getUsedVariables();
                  std::vector<codon::ir::Var *> elif_arg_2 =
                      elif_cond_2_call->back()->getUsedVariables();
                  auto elif_arg_1_name = elif_arg_1[0]->getName();
                  auto elif_arg_2_name = elif_arg_2[0]->getName();
                  std::string arg2_00 = "";
                  std::string arg2_01 = "";
                  ...
                  std::string arg2_05 = "";
                  arg2_00 = "0";
                  std::vector<codon::ir::Value *> all_args_2 =
                      m_call_2->getUsedValues();
                  ...
                  if (elif_cond_func_name == "__eq__" &&
                      ...
                      ((elif_arg_2_name == match_arg_1 ||
                        elif_arg_2_name == match_arg_2))) {
                    auto *return_val_2_1 =
                        cast<ReturnInstr>(cast<SeriesFlow>(elif_return_2)->back())
                            ->getValue();
                    auto *return_val_2_2 =
                        cast<ReturnInstr>(cast<SeriesFlow>(true_return_2)->back())
                            ->getValue();
                    auto *return_val_2_3 =
                        cast<ReturnInstr>(cast<SeriesFlow>(else_branch_2)->back())
                            ->getValue();
                    std::vector<codon::ir::Var *> ret_val_1 =
                        return_val_2_1->getUsedVariables();
                    auto ret_val_1_name = ret_val_1[0]->getName();
                    std::vector<codon::ir::Var *> ret_val_2 =
                        return_val_2_2->getUsedVariables();
                    auto ret_val_2_name = ret_val_2[0]->getName();
                    std::vector<codon::ir::Var *> ret_val_3 =
                        return_val_2_3->getUsedVariables();
                    auto ret_val_3_name = ret_val_3[0]->getName();
            ...
            auto *arg2_fr = cast<CallInstr>(
                arg2_inst->front());            // the left side operand + its row index
            auto *arg2_var = arg2_fr->front();  // the left side variable
            auto *arg2_row = arg2_fr->back();   // the left side row index
      ...
      if (main_args != 0) {
        ...
        if (arg3_inst == NULL) {
          ...
          auto max_att_6 =
              util::hasAttribute(max_st_6, "std.vectron.dispatcher.vectron_max");
          if (max_att_6) {
            params_24 = "0";
            params_25 = "0";
            params_26 = "0";
            ...
            if (mx_arg_3_2 == NULL) {
              ...
              auto mx_arg_3_2_func =
                  util::getFunc(mx_arg_3_2->getCallee())->getUnmangledName();
            ...
            if (mx_arg_3_3 == NULL) {
              ...
              auto mx_arg_3_3_func =
                  util::getFunc(mx_arg_3_3->getCallee())->getUnmangledName();
            ...
            auto mx_arg_3_4_name =
                util::getFunc(cast<CallInstr>(mx_arg_3_4)->getCallee())
                    ->getUnmangledName();
            if (mx_arg_3_4_name == "__add__" || mx_arg_3_4_name == "__sub__") {
              ...
              auto *mx_arg_3_4_left_part_1_1 =
                  cast<CallInstr>(mx_arg_3_4_left_part_1)->front();
              std::vector<codon::ir::Var *> part_1_lst =
                  mx_arg_3_4_left_part_1_1->getUsedVariables();
              auto part_1_lst_name = part_1_lst[0]->getName();
              mx3_03 = part_1_lst_name;
              auto *mx_arg_3_4_left_part_1_2 =
                  cast<CallInstr>(mx_arg_3_4_left_part_1)->back();
              if (cast<CallInstr>(mx_arg_3_4_left_part_1_2) == NULL) {
                mx3_04 = "0";
              } else {
                auto mx_arg_3_4_left_inst = util::getFunc(
                    cast<CallInstr>(mx_arg_3_4_left_part_1_2)->getCallee());
              ...
              if (cast<CallInstr>(mx_arg_3_4_part_2) == NULL) {
                ...
                auto *mx_arg_3_4_left_part_2 =
                    util::getFunc(cast<CallInstr>(mx_arg_3_4_part_2)->getCallee());
                auto mx_arg_3_4_part_2_inst =
                    mx_arg_3_4_left_part_2->getUnmangledName();
                ;
            ...
            auto mx_arg_3_5_name =
                util::getFunc(cast<CallInstr>(mx_arg_3_5)->getCallee())
                    ->getUnmangledName();
            if (mx_arg_3_5_name == "__add__" || mx_arg_3_5_name == "__sub__") {
              ...
              auto *mx_arg_3_5_left_part_1_1 =
                  cast<CallInstr>(mx_arg_3_5_left_part_1)->front();
              std::vector<codon::ir::Var *> part_1_lst =
                  mx_arg_3_5_left_part_1_1->getUsedVariables();
              auto part_1_lst_name = part_1_lst[0]->getName();
              mx3_07 = part_1_lst_name;
              auto *mx_arg_3_5_left_part_1_2 =
                  cast<CallInstr>(mx_arg_3_5_left_part_1)->back();
              if (cast<CallInstr>(mx_arg_3_5_left_part_1_2) == NULL) {
                mx3_08 = "0";
              } else {
                auto mx_arg_3_5_left_inst = util::getFunc(
                    cast<CallInstr>(mx_arg_3_5_left_part_1_2)->getCallee());
              ...
              if (cast<CallInstr>(mx_arg_3_5_part_2) == NULL) {
                ...
                auto *mx_arg_3_5_left_part_2 =
                    util::getFunc(cast<CallInstr>(mx_arg_3_5_part_2)->getCallee());
                auto mx_arg_3_5_part_2_inst =
                    mx_arg_3_5_left_part_2->getUnmangledName();
                ;
            ...
            std::map<std::string, std::string> mx3_attributes{
                {"mx3_00", mx3_00}, {"mx3_01", mx3_01}, {"mx3_02", mx3_02},
                {"mx3_03", mx3_03}, {"mx3_04", mx3_04}, {"mx3_05", mx3_05},
                {"mx3_06", mx3_06}, {"mx3_07", mx3_07}, {"mx3_08", mx3_08},
                {"mx3_09", mx3_09}, {"mx3_10", mx3_10}};
            ...
            if (arg3_func_name == "__add__" || arg3_func_name == "__sub__" ||
                arg3_func_name == "__mul__" || arg3_func_name == "__div__") {
              auto *arg3_op =
                  cast<CallInstr>(arg3_inst->front()); // the left side operand
              auto *arg3_fr = cast<CallInstr>(
                  arg3_op->front()); // the left side variable + its row index
              ...
              std::vector<codon::ir::Var *> arg3_var_name =
                  arg3_var->getUsedVariables();
              auto *arg3_row_instr =
                  cast<CallInstr>(arg3_row); // is arg3 row index a math op?
              auto *arg3_col_instr =
                  cast<CallInstr>(arg3_col); // is arg3 row index a math op?
              auto arg3_fr_name =
                  arg3_var_name[0]->getName(); // left side variable's name
              ...
              if (arg3_row_instr != NULL) {
                ...
                std::vector<codon::ir::Var *> arg3_row_var =
                    arg3_row_instr->front()->getUsedVariables();
              ...
              std::string arg3_col_var_name = ""; // taking care of the column index of
                                                  // the first (left side) operand
              if (arg3_col_instr != NULL) {
                ...
                std::vector<codon::ir::Var *> arg3_col_var =
                    arg3_col_instr->front()->getUsedVariables();
                ...
                if (arg3_num_col_inst == NULL) {
                  ...
                  std::vector<codon::ir::Var *> arg_3_num_col_left_vars =
                      arg_3_num_col_left->getUsedVariables();
                  ...
                  if (arg_3_num_col_right_inst == NULL) {
                    std::vector<codon::ir::Var *> arg_3_num_col_right_vars =
                        arg_3_num_col_right->getUsedVariables();
                    auto arg_3_num_col_right_name =
                        arg_3_num_col_right_vars[0]->getName();
                    ...
                    std::vector<codon::ir::Var *> arg3_num_col_right_left_vars =
                        arg3_num_col_right_left->getUsedVariables();
                    auto arg3_num_col_right_left_name =
                        arg3_num_col_right_left_vars[0]->getName();
                    ...
                    auto *arg3_num_col_right_func =
                        util::getFunc(arg_3_num_col_right_inst->getCallee());
                    auto arg3_num_col_right_func_name =
                        arg3_num_col_right_func->getUnmangledName();
                    ...
                    std::string oss_arg3_num_col_right_right_str =
                        oss_arg3_num_col_right_right.str();
                    params_28 += oss_arg3_num_col_right_right_str;
                  }
                }
              } else {
                std::vector<codon::ir::Var *> arg3_col_var =
                    arg3_col->getUsedVariables();
                arg3_col_var_name = arg3_col_var[0]->getName();
                if (var_str_outer == arg3_col_var_name) {
                  params_27 = "0";
                } else {
              ...
              if (arg3_func_name == "__sub__") {
                ...
                if (cast<CallInstr>(arg3_const) == NULL) {
                  if (cast<TernaryInstr>(arg3_const) == NULL) {
                    ...
                    // auto *match_arg_1 =
                    // (cast<CallInstr>(m_call_3->front())->front()); auto *match_arg_2
                    // = cast<CallInstr>(m_call_3->back())->front();
                    ...
                    if (cast<CallInstr>(arg_3_1_index) != NULL) {
                      ...
                      auto arg_3_1_index_func_name =
                          arg_3_1_index_func->getUnmangledName();
                    ...
                    if (cast<CallInstr>(arg_3_2_index) != NULL) {
                      ...
                      auto arg_3_2_index_func_name =
                          arg_3_2_index_func->getUnmangledName();
                    ...
                    std::map<std::string, std::string> arg3_attributes{
                        {"arg3_00", arg3_00}, {"arg3_01", arg3_01},
                        {"arg3_02", arg3_02}, {"arg3_03", arg3_03},
                        {"arg3_04", arg3_04}, {"arg3_05", arg3_05}};
                  ...
                  auto match_att_3 = util::hasAttribute(
                      m_call_3_func, "std.vectron.dispatcher.vectron_cmp");
                  if (match_att_3) {
                    std::vector<codon::ir::Value *> body_3 =
                        m_call_3_func->getUsedValues();
                    auto match_arg_1 = m_call_3_func->arg_front()->getName();
                    auto match_arg_2 = m_call_3_func->arg_back()->getName();

                    // auto *true_branch_3 =
                    // cast<IfFlow>(cast<SeriesFlow>(body_3[0])->back())->getCond();
                    auto *true_return_3 =
                        cast<IfFlow>(cast<SeriesFlow>(body_3[0])->back())
                            ->getTrueBranch();

                    auto *elif_branch_3_series =
                        cast<IfFlow>(cast<SeriesFlow>(body_3[0])->back())
                            ->getFalseBranch();
                    auto *elif_branch_3 =
                        cast<IfFlow>(cast<SeriesFlow>(elif_branch_3_series)->back())
                            ->getCond();
                    auto *elif_return_3 =
                        cast<IfFlow>(cast<SeriesFlow>(elif_branch_3_series)->back())
                            ->getTrueBranch();
                    ...
                    std::vector<codon::ir::Var *> elif_arg_1 =
                        elif_cond_3_call->front()->getUsedVariables();
                    std::vector<codon::ir::Var *> elif_arg_2 =
                        elif_cond_3_call->back()->getUsedVariables();
                    auto elif_arg_1_name = elif_arg_1[0]->getName();
                    auto elif_arg_2_name = elif_arg_2[0]->getName();
                    std::string arg3_00 = "";
                    std::string arg3_01 = "";
                    ...
                    std::string arg3_05 = "";
                    arg3_00 = "0";
                    std::vector<codon::ir::Value *> all_args_3 =
                        m_call_3->getUsedValues();
                    ...
                    if (elif_cond_func_name == "__eq__" &&
                        ...
                        ((elif_arg_2_name == match_arg_1 ||
                          elif_arg_2_name == match_arg_2))) {
                      auto *return_val_3_1 =
                          cast<ReturnInstr>(cast<SeriesFlow>(elif_return_3)->back())
                              ->getValue();
                      auto *return_val_3_2 =
                          cast<ReturnInstr>(cast<SeriesFlow>(true_return_3)->back())
                              ->getValue();
                      auto *return_val_3_3 =
                          cast<ReturnInstr>(cast<SeriesFlow>(else_branch_3)->back())
                              ->getValue();
                      std::vector<codon::ir::Var *> ret_val_1 =
                          return_val_3_1->getUsedVariables();
                      auto ret_val_1_name = ret_val_1[0]->getName();
                      std::vector<codon::ir::Var *> ret_val_2 =
                          return_val_3_2->getUsedVariables();
                      auto ret_val_2_name = ret_val_2[0]->getName();
                      std::vector<codon::ir::Var *> ret_val_3 =
                          return_val_3_3->getUsedVariables();
                      auto ret_val_3_name = ret_val_3[0]->getName();
              ...
              auto *arg3_fr = cast<CallInstr>(
                  arg3_inst->front()); // the left side operand + its row index
              auto *arg3_var = arg3_fr->front();  // the left side variable
              auto *arg3_row = arg3_fr->back();   // the left side row index
    ...
    if (arg1_inst != NULL && !max_att_7) {
      ...
      auto *arg1_fr =
          cast<CallInstr>(arg1_op->front()); // the left side variable + its row index
      if (arg1_fr != NULL) {
        auto *arg1_var = arg1_fr->front(); // the left side variable
    ...
    if (arg2_inst != NULL && !max_att_8) {
      ...
      auto *arg2_fr =
          cast<CallInstr>(arg2_op->front()); // the left side variable + its row index
      if (arg2_fr != NULL) {
        auto *arg2_var = arg2_fr->front(); // the left side variable
    ...
    if (r_end != r_mid[1]) {
      ...
      if (arg3_inst != NULL && !max_att_9) {
        ...
        auto *arg3_fr =
            cast<CallInstr>(arg3_op->front()); // the left side variable + its row index
        if (arg3_fr != NULL) {
          auto *arg3_var = arg3_fr->front(); // the left side variable
          ...
          std::vector<codon::ir::Var *> arg3_vars =
              arg3_op->front()->getUsedVariables();
          auto arg3_var_name = arg3_vars[0]->getName();
          params_32 = arg3_var_name;
        }
      } else {
    ...
    std::map<std::string, std::string> params_attributes{
        {"params_00", params_00}, {"params_01", params_01}, {"params_02", params_02},
        {"params_03", params_03}, {"params_04", params_04}, {"params_05", params_05},
        {"params_06", params_06}, {"params_07", params_07}, {"params_08", params_08},
        {"params_09", params_09}, {"params_10", params_10}, {"params_11", params_11},
        {"params_12", params_12}, {"params_13", params_13}, {"params_14", params_14},
        {"params_15", params_15}, {"params_16", params_16}, {"params_17", params_17},
        {"params_18", params_18}, {"params_19", params_19}, {"params_20", params_20},
        {"params_21", params_21}, {"params_22", params_22}, {"params_23", params_23},
        {"params_24", params_24}, {"params_25", params_25}, {"params_26", params_26},
        {"params_27", params_27}, {"params_28", params_28}, {"params_29", params_29},
        {"params_30", params_30}, {"params_31", params_31}, {"params_32", params_32},
        {"params_33", params_33}};
