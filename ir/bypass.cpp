...
void byPass::transform(ReturnInstr *v) {
  ...
  if (!att_bypass) {
        ...
        if (in_function && in_vectron_calc) {
          if (trimmed_line.find("return ") != std::string::npos) {
            ...
            std::string extracted_text =
                trimmed_line.substr(trimmed_line.find("return ") + 7);
    ...
    if (extracted_lines.empty()) {
      std::cerr
          << "Error: No return statement found in function decorated with @vectron\n";
      return;
    }
    // int bp;
    // pf->setAttribute(bp, -1);
    std::map<std::string, std::string> attributes{{"Value", "-1"},
                                                  {"Expression", extracted_lines[0]}};
    ...
    std::map<std::string, std::string> attributes{{"Value", z_value_str},
                                                  {"Expression", "score"}};
    globalAttributes["bypass"] = attributes;
  }
}
