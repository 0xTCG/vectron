...
void align(std::vector<int16_t> &scores, dp_mat &matrix,
    ...
    for (int t = 0; t < QUANTITY; ++t) {
        ...
        for (int16_t i = 1; i < SIZE + 1; ++i) {
            matrix[0][i] = gap_o + gap_e_c * i;
            matrix[i][0] = gap_o + gap_e_r * i;
        }
        ...
        for (int16_t i = 1; i < SIZE + 1; ++i) {
            for (int16_t j = 1; j < SIZE + 1; ++j) {
                ...
                int16_t diagonal_value = matrix[i - 1][j - 1] + value;
                int16_t top_value = matrix[i - 1][j] + value;
                int16_t left_value = matrix[i][j - 1] + value;
