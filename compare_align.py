...
def orig(T, params):     
    with time.timing("total"):    
        for t in T:
            query, target = t
            aln = query.align(target, a=params["a"], b=params["b"], ambig=params["ambig"], gapo=params["gapo"], gape=params["gape"], zdrop=params["zdrop"], bandwidth=params["bandwidth"], score_only=True, end_bonus=params["end_bonus"])
            print(aln.score)
