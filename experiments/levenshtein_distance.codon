...
@vectron_calc
def orig_foo(h, m):
    q = [[0 if (_ == 0 and __ == 0) else (_ * (1)) if (__ == 0 and _ > 0) else (__ * (1)) if (_ == 0 and __ > 0) else 0 for __ in range(len(m) + 1)] for _ in range(len(h) + 1)]   
    for i in range(1, len(m) + 1):
        for j in range(1, len(h) + 1):       
            if j - i <= -1 or j - i >= 1:
                ...
                q[i][j] = max(q[i - 1][j - 1] + match_func_foo(m[i - 1], 0, 1, 1, h[j - 1]), q[i - 1][j] + 1, q[i][j - 1] + 1)
    print(q[-1][-1])
    return q[-1][-1]
